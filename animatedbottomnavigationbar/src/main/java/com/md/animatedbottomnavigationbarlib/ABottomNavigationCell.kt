...
import kotlinx.android.synthetic.main.navigation_cell.view.*
...
@Suppress("unused")
class ABottomNavigationCell : RelativeLayout, LayoutContainer {
...
var title: String? = EMPTY_VALUE
        set(value) {
            ...
            if (allowDraw) {
                if (title != null && title == EMPTY_VALUE) {
                    ...
                    val scale = if (title?.isEmpty() == true) 0.5f else 1f
                    tv_title.scaleX = scale
                    tv_title.scaleY = scale
                }
            }
        }
    ...
    private fun initializeView() {
        allowDraw = true
        containerView = LayoutInflater.from(context).inflate(R.layout.navigation_cell, this)
        draw()
    }
